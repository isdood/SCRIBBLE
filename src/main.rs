...
fn kernel_main(boot_info: &'static BootInfo) -> ! {
    ...
    match freezer::login("slug") {
        true => {
            splat::log(SplatLevel::Info, &format!(
                "System activated\n\
Kernel Version: {}\n\
Boot Time: {}\n\
Current User: {}",
env!("CARGO_PKG_VERSION"),
        ...
        false => {
            splat::log(SplatLevel::Critical, &format!("Initial system thaw failed"));
    ...
    loop {
        ...
        if consecutive_anomalies > 5 {
            freezer::increment_freeze_count();
...
fn init_system(boot_info: &'static BootInfo) -> Result<(), &'static str> {
    gdt::init();
    memory::init(boot_info).map_err(|_| "Memory initialization failed")?;
    Ok(())
}
...
fn check_system_status(stats: &SystemMetrics, consecutive_anomalies: &mut u32) {
    ...
    if memory_usage > 90.0 {
        splat::log(SplatLevel::Critical, &format!(
