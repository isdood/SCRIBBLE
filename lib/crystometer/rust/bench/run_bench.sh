...
run_benchmark() {
    ...
    echo "$name,$valid_samples,$mean,$stddev" >> results/rust/latest_results.csv
}

# Check CPU throttling
check_cpu_throttling
if [ $? -eq 1 ]; then
    printf "${CYAN}Warning: Results may be affected by CPU frequency scaling${NC}\n"
fi
...
rm -f results/rust/latest_results.csv
...
generate_html_report "rust" "$TIMESTAMP"

# Print summary
total_ops=$(awk -F',' '{sum+=$2} END {print sum}' results/rust/latest_results.csv)
total_time=$(awk -F',' '{sum+=$2*$3} END {printf "%.2f", sum/1000000}' results/rust/latest_results.csv)
avg_time=$(awk -F',' '{sum+=$3} END {printf "%.2f", sum/NR}' results/rust/latest_results.csv)
